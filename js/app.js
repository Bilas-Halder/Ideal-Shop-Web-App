// Modal Informations for details
const showDetails = id => {
  fetch(`https://fakestoreapi.com/products/${id}`)
    ...
    .then(data => {
      ...
      document.querySelector("#modal-price").innerText = `Price: $ ${data.price}`;
      document.getElementById("modal-AddToCart-btn").addEventListener("click", () => {
        addToCart(id, data.price);
      });
    })
    .catch(error => {
      // showing error massage for any error
      document.querySelector(".modal-title").innerHTML = `<h3 class="text-center" style="color:red">Error Occurred</h3>`;
      document.querySelector("#description").innerHTML = `<h3 class="text-center" style="color:red">Please Try Again</h3>`;
      document.querySelector("#modal-price").innerText = ``;
      document.getElementById("modal-AddToCart-btn").addEventListener("click", () => {
        addToCart("string-id", 0);
      });
    })
};

// stars setting for any product
const getStars = rating => {
  let p = `<p>`;
  let innerHtml = '';
  let i = 0;
  const fullRate = Math.floor(rating.rate);
  const remaining = rating.rate - fullRate;
  // filled star for stars of this product
  while (i < fullRate) {
    innerHtml += `<i class="fas fa-star"></i>`;
    i++;
  }
  // half filled star for fractions
  if (remaining > 0) {
...
const addToCart = (id, price) => {
  typeof id !== 'string' ? count = count + 1 : '';
...
// fetch url/ api was not working while i was debuging this
// so i store the dataa instead of fetching
const loadProducts = () => {
